...
dir_to_search = r"c:\Users\Philippe\Desktop\Quanto falta\quanto-falta-server\dashboard"
...
extensions = ('.html', '.js', '.json', '.ts', '.tsx', '.css', '.env', '.example')
...
for root, dirs, files in os.walk(dir_to_search):
    if 'node_modules' in root or 'dist' in root or 'out' in root or '.next' in root or '.git' in root:
        continue
    for file in files:
        if file.endswith(extensions) or file.startswith('.env'):
